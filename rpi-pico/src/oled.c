...
static inline void sendCMD(const byte *cmd, byte len) {
    #if USE_I2C == 1
...
static inline void sendData(byte *data, word len) {
    #if USE_I2C == 1
...
inline static void drawFastRawHLine(word x, word y, word w, byte color) {memset(buff + ((y * WIDTH + x)), color, w/2);}
inline void drawHLine(word x, word y, word w, byte color) {drawLine(x, y, w, y, color);}
...
inline void drawVLine(word x, word y, word h, byte color) {drawLine(x, y, x, h, color);}
...
void fillRect(byte x, byte y, byte w, byte h, byte color) {
    ...
    for(word i = y; i < y+h; i++) drawLine(x, i, (x+w), i, color);
}


inline void charSize(byte size) {text.size = size;}
inline void charColor(byte color) {text.color = color;}
...
inline void sendc(byte x, byte y, char c) {putChar(x, y, c, text.color, 0x00, text.size);} 
