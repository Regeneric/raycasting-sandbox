...
extern "C" {
    // C declaration for Fortran function raycast()
    float raycast_(int *fov, int *cell, int *mapW, int *mapH, int *map, float *playerX, float *playerY, float *playerAng, float *rayX, float *rayY, int *draw);
}
...
void Ray::cast(float f, Player *player, Wall *map, sf::RenderWindow *window) {
    ...
    std::cout << "PA: " << playerAngle << std::endl;

    // Fortran function
    float dist = raycast_(&fov, &cell, &mapW, &mapH, mapGrid, &playerX, &playerY, &playerAngle, &rayX, &rayY, &draw);

    std::cout << "DIST: " << dist << std::endl;
    std::cout << "RX: " << rayX << " ; RY: " << rayY << std::endl;
    ...
    //     }
        ...
        window->draw(sw::Line(sf::Vector2f(playerX, playerY), sf::Vector2f(rayX, rayY), 0.0f, sf::Color::Red));
